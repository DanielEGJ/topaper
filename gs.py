...
archConv = np.array([32, 120, 7, 2, 3]) #min kernels, max kernels, total layers, min size kernel, max size kernel
...
class sample:
    ...
    def getArch(self):
        return self.archConv.copy(), self.archConvKernels.copy(), self.archConvParamsDisc.copy(), self.archDen.copy(), self.archDenParamsDisc.copy()

    def getParams(self):
        return self.paramsCont.copy(), self.paramsDisc.copy()
...
class GS:
    def __init__(self, sizePopulation, sizeTournament, matingPoolSize, mutationRate, save=True, history=True, historySamples=10, debug=False, debugSave=False):
        self.population = []
        self.newPopulation = []
        self.matingPool = []
    ...
    def makeHistory(self):
        with open(f'models/gen{self.gen}/history.txt', 'w') as f:
                f.write(f'Top {self.historySamples} Individuals.\n')
    ...
    def runGeneticStrategies(self, generations=10):
...
GeneticStrategies.runGeneticStrategies(
    generations = 25)
